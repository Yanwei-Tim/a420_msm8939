...
static bool wipe_data(int should_confirm, Device* device) {
    ...
    FILE* heart_pulse_res = fopen("/data/misc/pulse_res", "a+");
    fclose(heart_pulse_res);
    return success;
}
...
main(int argc, char **argv) {
    ...
    int arg;
    bool need_swith_user = false;//wangqing.yu add for userswith when factory reset
...
error:
    ...
    switch (after) {
        ...
        default:
            ...
            ui->Print("Rebooting...\n");
            if(need_swith_user)
            property_set(ANDROID_RB_PROPERTY, "reboot,userswitch");
            else
