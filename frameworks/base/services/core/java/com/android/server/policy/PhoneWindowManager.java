...
/*add by yangzhiming 20170106 for bug 0014104 start*/
import com.android.server.lights.Light;
import com.android.server.lights.LightsManager;
/*add by yangzhiming 20170106 for bug 0014104 end*/
...
public class PhoneWindowManager implements WindowManagerPolicy {
   ...
   /**
     * The full dialer class name.
     */
    private static final String DIALER_CLASS_NAME = "com.android.dialer.DialtactsActivity";
    ...
    volatile boolean mPoundKeyHandled;
    ...
    /*add by yangzhiming 20170106 for bug 0014104 start*/
    Light mLight;
    int mButtonLightTimeout;
    int mButtonBrightness;
    boolean mIsSupportLightButton = "msm8916_64_a420".equals(SystemProperties.get("ro.product.name"));
    boolean isButtonLightInited;
    static final int MSG_TURN_OFF_BUTTON_LIGHT = 101;
    /*add by yangzhiming 20170106 for bug 0014104 end*/
    ...
    private boolean mScreenshotChordHomeKeyTriggered;
    private long mScreenshotChordHomeKeyTime;
	private boolean mScreenshotChordEndCallTriggered;
    private long mScreenshotChordEndCallTime;

/*add for cts-verifier by wxm,start*/
...
/*add for cts-verifier by wxm,end*/
    ...
    //reset def A B  key action by zzj start
    private static final int MSG_BBL_TIMEOUT = 16;
    private static final int MSG_SARTACTIVITY_KEY_A = 17;
    private static final int MSG_SARTACTIVITY_KEY_B = 18;
    //reset def A B  key action by zzj end
    boolean mWifiDisplayConnected = false;
    int mWifiDisplayCustomRotation = -1;
    //reset def A B  key action by zzj start
    private static final String ACTION_SHORTCUTS_CHANGE = "get_allapps_action_shorts_change";
    private static final String ACTION_SHORTCUTS_KEY = "get_allapps_action_shorts_key";
    private static String AKEY_ACTIVITYNAME = "com.ublox.ucenter.Splash";
    private static String AKEY_PACKAGENAME = "com.ublox.ucenter";
    private static String BKEY_PACKAGENAME = "org.codeaurora.snapcam";
    private static String BKEY_ACTIVITYNAME = "com.android.camera.CameraLauncher";
    private boolean mSensorEnabled;
    // add switch key by zzj start
    private boolean mIsFolderOpen = true;
    private boolean mIsDoubleScreen = SystemProperties.get("ro.config.doublescreen","0").equals("1");
    private final static String FOLDER_STATUS_PATH = "sys/class/sec_flip/status/flipStatus";
    private final static String CLOSE_ANALOG_WITH_BROADCAST = "android.intent.action.close_analogclock";
    //add switch key by zzj end

    //add this for double-tap to camera.by zj.
    private int mRepeatCount = 0;

    //start ext_ant  action
    private final String extouchaction = "com.lovdream.exttouch";
    private final String exrelaseaction = "com.lovdream.extrelease";
    //end 
    BroadcastReceiver mABShortcutsReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
                if(ACTION_SHORTCUTS_CHANGE.equals(intent.getAction())){
                        ArrayList localArrayList = new ArrayList();
                        localArrayList = intent.getStringArrayListExtra(ACTION_SHORTCUTS_KEY);
                        if(localArrayList.size() > 1){
                                AKEY_PACKAGENAME = (String) localArrayList.get(0);
                                AKEY_ACTIVITYNAME = (String) localArrayList.get(1);
                        }
                        if(localArrayList.size() > 3){
                                BKEY_PACKAGENAME = (String) localArrayList.get(2);
                                BKEY_ACTIVITYNAME = (String) localArrayList.get(3);
                        }
                }
        }
    };
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_UPDATE_DREAMING_SLEEP_TOKEN:
                    ...
                    break;
		//fix bug about key light by zzj start
		case MSG_BBL_TIMEOUT:
                    setBLNStatus(0);
		    break;
		//fix bug about key light by zzj end
		//reset def A B  key action by zzj start
		case MSG_SARTACTIVITY_KEY_A:
                              if(isActivityShow(AKEY_ACTIVITYNAME) || AKEY_ACTIVITYNAME.startsWith("none")){
                                break;
                                                 }
                                try{
                                        Intent intentA = new Intent();
                                        intentA.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                                        intentA.setComponent(new ComponentName(AKEY_PACKAGENAME, AKEY_ACTIVITYNAME));
                                        mContext.startActivity(intentA);
                                }catch(Exception e){
                                        Log.e(TAG,"can't startActivity A ---- "+e.toString());
                                }
                    break;
                case MSG_SARTACTIVITY_KEY_B:
                         if(isActivityShow(BKEY_ACTIVITYNAME) || BKEY_ACTIVITYNAME.startsWith("none")){
                                break;
                         }
                        try{
                                Intent intentB = new Intent();
                                        intentB.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                                        intentB.setComponent(new ComponentName(BKEY_PACKAGENAME, BKEY_ACTIVITYNAME));
                                        mContext.startActivity(intentB);
                        }catch(Exception e){
                                        Log.e(TAG,"can't startActivity B ---- "+e.toString());
                                }
                    break;
		//reset def A B  key action by zzj end
            }
   ...
   //reset def A B  key action by zzj start 
    private boolean isActivityShow(String activityName){
        boolean show = false;
        if(!isLauncherRunnig(mContext)){
                show = true;
        }
        return show;
    }
    ...
    class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            resolver.registerContentObserver(Settings.Global.getUriFor(
                    ...
                    UserHandle.USER_ALL);
	//add hall key switch by zzj start
           resolver.registerContentObserver(Settings.Global.getUriFor("sensor_enabled"), false, this,
                    UserHandle.USER_ALL);
        //add hall key switch by zzj end
            updateSettings();
        }

        @Override public void onChange(boolean selfChange) {
	 //add hall key switch by zzj start
            mSensorEnabled = Settings.Global.getInt(mContext.getContentResolver(), "sensor_enabled", 0) == 1;
         //add hall key switch by zzj end
            updateSettings();
        ...
        if (!mPowerKeyHandled) {
            if (interactive) {
                ...
                if(mIsSupportLightButton){
                    setButtonLightEnabled(false);//add by yangzhiming 20170106 for bug 0014105
                }
            } else {
                wakeUpFromPowerKey(event.getDownTime());
                if(mIsSupportLightButton){
                    setButtonLightEnabled(true);//add by yangzhiming 20170106 for bug 0014105
                }
...
/*add for cts-verifier by wxm,start*/
	private void interceptStopLockTaskMode(){
		if(mStopLockTaskModeMenuKeyTriggered && mStopLockTaskModeBackKeyTriggered){
			...
			if(now <= mStopLockTaskModeMenuKeyTime + SCREENSHOT_CHORD_DEBOUNCE_DELAY_MILLIS
					&& now <= mStopLockTaskModeBackKeyTime + SCREENSHOT_CHORD_DEBOUNCE_DELAY_MILLIS){
...
/*add for cts-verifier by wxm,end*/
    ...
    private void interceptScreenshotChord2() {
        if (mScreenshotChordEnabled
                && mScreenshotChordHomeKeyTriggered && mScreenshotChordEndCallTriggered) {
            final long now = SystemClock.uptimeMillis();
            if (now <= mScreenshotChordHomeKeyTime + SCREENSHOT_CHORD_DEBOUNCE_DELAY_MILLIS
                    && now <= mScreenshotChordEndCallTime
...
/*add for cts-verifier by wxm,end*/	
	...
	private boolean isInLauncher(){
		return "com.cooee.phenix".equals(mTopFullscreenOpaqueWindowState.getOwningPackage());
	}

	private final Runnable mPoundLongPress = new Runnable(){
		@Override
		public void run() {
			mPoundKeyHandled = true;
			try{
				final boolean silentModeOn = mAudioManager.getRingerMode() != AudioManager.RINGER_MODE_NORMAL;
                mAudioManager.setRingerMode(silentModeOn ? AudioManager.RINGER_MODE_NORMAL : AudioManager.RINGER_MODE_VIBRATE);
				GlobalActions.showAudioMessage(mContext,silentModeOn);
			}catch(Exception e){
				e.printStackTrace();
			}
		}
...
/*add for cts-verifier by wxm,end*/
    ...
    void showGlobalActionsInternal() {
        sendCloseSystemWindows(SYSTEM_DIALOG_REASON_GLOBAL_ACTIONS);
        if("msm8916_64_a420".equals(SystemProperties.get("ro.product.name"))){
            try{
                showShutdownUI();
                return;
            } catch(Exception e){
    ...
    private void showShutdownUI(){
        Intent intent = new Intent();
        intent.setClassName("com.lovdream.shutdown.shutdownui","com.lovdream.shutdown.shutdownui.ShutdownActivity");
    ...
    public void init(Context context, IWindowManager windowManager,
		...
		//reset def A B  key action by zzj start
		filter = new IntentFilter(ACTION_SHORTCUTS_CHANGE);
        context.registerReceiver(mABShortcutsReceiver, filter);
		//reset def A B  key action by zzj end
	...
	filter.addAction("intent.action.ANALOGFLAGSETTINGS");
        filter.addAction("intent.action.ANALOG_FLAG_CLOSED");
	...
	if(mIsDoubleScreen) {
            ...
            try {
                reader = new FileReader(FOLDER_STATUS_PATH);
                int n = reader.read();
                if (n == '0') {
                    mIsFolderOpen = false;
                } else {
                ...
                SystemProperties.set("persist.sys.folderopen", mIsFolderOpen ? "1":"0");
            } catch (IOException ex) {
                Slog.w(TAG, "Couldn't read flip state from " + FOLDER_STATUS_PATH + ": " + ex);
   ...
   //fix bug about key light by zzj start
    private void setBLNStatus(int status) {
                File file = new File("sys/class/leds/button-backlight/brightness");
	...
	android.util.Log.i("....zzj","--------interceptKeyBeforeDispatching------"+keyCode);
	//fix bug 13668 by zzj start
	 final boolean hapticsDisabled = Settings.System.getIntForUser(mContext.getContentResolver(),
                Settings.System.HAPTIC_FEEDBACK_ENABLED, 1, UserHandle.USER_CURRENT) == 1;
	if(hapticsDisabled && !keyguardOn && !mIsDoubleScreen){
	    if( keyCode == KeyEvent.KEYCODE_MENU || keyCode == KeyEvent.KEYCODE_HOME || keyCode == KeyEvent.KEYCODE_BACK){
                /* modify by yangzhiming 20161228 for bug 0014054 start*/
                if (down && 0 == repeatCount){
                    mVibrator.vibrate(20);
                }
                /* modify by yangzhiming 20161228 for bug 0014054 end*/
		//fix bug about key light by zzj start
		mHandler.removeMessages(MSG_BBL_TIMEOUT);
		setBLNStatus(1);
	        mHandler.sendMessageDelayed(mHandler.obtainMessage(MSG_BBL_TIMEOUT),
                        4000);	
		//fix bug about key light by zzj end
	    }
	}
	//fix bug 13668 by zzj end
        if (DEBUG_INPUT) {
        ...
        //start fix bug of 13340  13431add by chr
      if(keyguardOn &&(keyCode == KeyEvent.KEYCODE_VOLUME_DOWN || keyCode == KeyEvent.KEYCODE_VOLUME_UP  || keyCode == KeyEvent.KEYCODE_MENU))
        {
        ...
        {
                Log.i("Demo","========KeyEvent.KEYCODE_EXT_ANT_TOUCH=============");
                Intent intent = new Intent(extouchaction);
                mContext.sendBroadcast(intent);
        }

        if(keyCode == KeyEvent.KEYCODE_F4 )
        {
             Log.i("Demo","=========KeyEvent.KEYCODE_EXT_ANT_RELEASE===============");
             Intent intent = new Intent(exrelaseaction);
             mContext.sendBroadcast(intent);
        }
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
            ...
            if (!down) {
                ...
                if (mDoubleTapOnHomeBehavior != DOUBLE_TAP_HOME_NOTHING
                        && !( (PolicyControl.getWindowFlags(win, null) & WindowManager.LayoutParams.FLAG_CIT_HOME_TEST) == 0) ) {
               ...
               //Home key pass in cit by zzj start    
                WindowManager.LayoutParams attrs = win != null ? win.getAttrs() : null;
                if (attrs != null && attrs.type != WindowManager.LayoutParams.TYPE_KEYGUARD) {
                	//### add this for jump to camera. by zj. start
                	if (isDoubleTap()) {
                		dispatchOpenCamera();
                	} else {
                		handleShortPressOnHome();
                	}
                	//### add this for jump to camera. by zj. end
                }else{
                    return 0;
               }
                  return -1;
            }
                //Home key pass in cit by zzj end
	...
	if(SystemProperties.getBoolean("sys.cit_keytest", false)){
                    return 0;
                 }
            if (!keyguardOn) {
                if (down && repeatCount == 0) {
	...
	//reset def A B  key action by zzj start 
                ...
                if (attrs != null) {
                    ...
                    if (type == WindowManager.LayoutParams.TYPE_KEYGUARD
                            || type == WindowManager.LayoutParams.TYPE_KEYGUARD_DIALOG || keyguardOn) {
                        // the "app" is keyguard, so give it the key
                        return 0;
                    }
                }
                mContext.sendBroadcast(new Intent("android.intent.action.SOFT_LEFT_UP_BUTTON").putExtra("isdown", down));
                if (down && repeatCount == 0){
                        mHandler.sendEmptyMessageDelayed(MSG_SARTACTIVITY_KEY_A, 600);
                }else if(down && repeatCount > 0){
                        mHandler.removeMessages(MSG_SARTACTIVITY_KEY_A);
                }
                ...
                mContext.sendBroadcast(new Intent("android.intent.action.SOFT_LEFT_DOWN_BUTTON").putExtra("isdown", down));
                if (down && repeatCount == 0){
                        mHandler.sendEmptyMessageDelayed(MSG_SARTACTIVITY_KEY_B, 600);
                }else if(down && repeatCount > 0){
                        mHandler.removeMessages(MSG_SARTACTIVITY_KEY_B);
                }
	...
	//reset def A B  key action by zzj end
         ...
         //start fix bug of 11676 add by chr
        if(down && repeatCount == 0)//fix bug of 12279 add by chr
        {
			...
			if(keyCode > 6 && keyCode < 17)
			{
				...
				int num = keyCode - 7;
				if(isDialerTopActivity()){
			
					Slog.w(TAG,"keyCode = " + num );
					
					Intent intent = new Intent("receiverlastnum");
    ...
    private boolean isDialerTopActivity() {
        ...
        if ((taskList != null)
                ...
                && (taskList.get(0).topActivity.getClassName().equals(DIALER_CLASS_NAME))) {
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        int result;
        boolean isShouldLightButton = true;//add by yangzhiming 20170106 for bug 0014104
        ...
        switch (keyCode) {
			...
			case KeyEvent.KEYCODE_HOME: {
				if (down) {
					if (interactive && !mScreenshotChordHomeKeyTriggered
							/*&& (event.getFlags() & KeyEvent.FLAG_FALLBACK) == 0*/) {
						mScreenshotChordHomeKeyTriggered = true;
						mScreenshotChordHomeKeyTime = event.getDownTime();
						mHandler.removeCallbacks(mEndCallLongPress);
						interceptScreenshotChord2();
					}
				} else {
					mScreenshotChordHomeKeyTriggered = false;
					cancelPendingScreenshotChordAction();
				}
 		 ...
 		 if(SystemProperties.getBoolean("sys.cit_keytest", false)){
                ...
                if (down) {
					...
					*/
					...
					if (interactive && !mScreenshotChordEndCallTriggered
							/*&& (event.getFlags() & KeyEvent.FLAG_FALLBACK) == 0*/) {
						mScreenshotChordEndCallTriggered = true;
						mScreenshotChordEndCallTime = event.getDownTime();
						interceptScreenshotChord2();
					}

                } else {
					mScreenshotChordEndCallTriggered = false;
					cancelPendingScreenshotChordAction();

                    mHandler.removeCallbacks(mEndCallLongPress);

					boolean handled = mEndCallKeyHandled || mScreenshotChordEndCallTriggered || mScreenshotChordHomeKeyTriggered;

					if(!handled){
						TelecomManager telecomManager = getTelecommService();
						boolean hungUp = false;
						if (telecomManager != null) {
							handled = telecomManager.endCall();
						}
					}
			...
			case KeyEvent.KEYCODE_POUND: {
				if (down) {
					if(interactive && isInLauncher()){
						mPoundKeyHandled = false;
						mHandler.postDelayed(mPoundLongPress,ViewConfiguration.get(mContext).getDeviceGlobalActionKeyTimeout());
					} else {
						mPoundKeyHandled = true;
					}
				} else {
					if (!mPoundKeyHandled){
						mHandler.removeCallbacks(mPoundLongPress);
					}
				}
				break;
			}

	    case KeyEvent.KEYCODE_DISPLAY_OFF: {
                Log.e(TAG,"case KeyEvent.KEYCODE_DISPLAY_OFF");
            }
		 ...
		 if(SystemProperties.getBoolean("sys.cit_keytest", false)){
                ...
                isShouldLightButton = false;//will be handled separately
                break;
            }
	  ...
	  //add hall key switch by zzj start
		...
		if(SystemProperties.getBoolean("sys.cit_keytest", false)){
                    return ACTION_PASS_TO_USER;
                      ...
                      if(down && mSensorEnabled){
                            mContext.sendBroadcast(new Intent("com.lovdream.KEYCODE_HALL_CLOSE"));
                            mPowerManager.goToSleep(event.getEventTime(),
                            PowerManager.GO_TO_SLEEP_REASON_POWER_BUTTON, 0);
                            isWakeKey = false;
                       }
		...
		if(SystemProperties.getBoolean("sys.cit_keytest", false)){
                    return ACTION_PASS_TO_USER;
                      ...
                      if(down && mSensorEnabled){
                       result &= ~ACTION_PASS_TO_USER;
                        isWakeKey = true;
                        mContext.sendBroadcast(new Intent("com.lovdream.KEYCODE_HALL_OPEN"));
                      }else{
                       isWakeKey = false;
                }
                break;
	...
	//add analog clock by zzj start
	    case KeyEvent.KEYCODE_FOLDER_CLOSE: {
		 if(SystemProperties.getBoolean("persist.sys.cittest", false)){
                    return ACTION_PASS_TO_USER;
                 }

		Log.e(TAG,"add KEYCODE_FOLDER_CLOSE,mIsFolderOpen = " + mIsFolderOpen);
		 openAnalog();
                if (down) {
                        TelecomManager telecomManager = getTelecommService();
                    boolean hungUp = false;
               final boolean isDropPhone=Settings.System.getInt(mContext.getContentResolver(),
                          "close_folder_drop_controllor", 0) == 1;
                    if (telecomManager != null && isDropPhone) {
                        hungUp = telecomManager.endCall();
                    }
                }
                SystemProperties.set("persist.sys.folderopen", "0");
                if(mIsFolderOpen){
                 mHandler.postDelayed(new Runnable() {
                                @Override
                                public void run() {
                                Intent intentclose = new Intent("com.android.lockscreen.folderclose");
                                    mContext.sendBroadcast(intentclose);
                                }
                        }, 600);
	     ...
	     case KeyEvent.KEYCODE_FOLDER_OPEN:{
                Log.e(TAG,"add KEYCODE_FOLDER_OPEN, mIsFolderOpen = " + mIsFolderOpen);
		 if(SystemProperties.getBoolean("persist.sys.cittest", false)){
                    return ACTION_PASS_TO_USER;
                 }
                ...
                if(!mIsFolderOpen){
                    mIsFolderOpen = true;
                    Intent intentopen = new Intent("com.android.lockscreen.folderopen");
                    mContext.sendBroadcast(intentopen);
                }
                Intent intent = new Intent();
                intent.setAction(CLOSE_ANALOG_WITH_BROADCAST);
                mContext.sendBroadcast(intent);
                isInAnalog = false;
                SystemProperties.set("persist.sys.folderopen", "1"); 
                final boolean isAnswerPhone =Settings.System.getInt(mContext.getContentResolver(),"folder_open_answer_controllor", 0) == 1; 
                if (down && isAnswerPhone) {
                         TelecomManager telecomManager = getTelecommService();
                     if (telecomManager != null) {
                         if (telecomManager.isRinging()) {
                             telecomManager.acceptRingingCall();
	...
	//add analog clock by zzj end
        ...
        /*add by yangzhiming 20170106 for bug 0014104 start*/
        if(mIsSupportLightButton && isShouldLightButton && down){
	...
	private boolean mAnaLogSettings;
	private boolean isInAnalog;
	public void openAnalog(){
           mAnaLogSettings = Settings.System.getInt(mContext.getContentResolver(), Settings.System.ANALOG_ENABLE, 0) ==0 ? false: true;
           if(mAnaLogSettings && !!mIsFolderOpen && !isInAnalog){
            ...
            try {
                   Intent intent = new Intent();
                   intent.setClassName("com.lovdream.analog", "com.lovdream.analog.MainActivity");
                   intent.setAction("android.intent.action.SHOWANALOGCLOCK");
    ...
    BroadcastReceiver mDreamReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } else if ("intent.action.ANALOGFLAGSETTINGS".equals(intent.getAction())) {
                mAnaLogSettings = intent.getBooleanExtra("showAnalogClock", false);
            } else if ("intent.action.ANALOG_FLAG_CLOSED".equals(intent.getAction())) {
                isInAnalog = false;
                if(!mIsFolderOpen){
                        Intent intentclose = new Intent("com.lovdream.analogclock.close");
                    mContext.sendBroadcast(intentclose);
                }
            }
        }
    ...
    /*add by yangzhiming 20170106 for bug 0014104 start*/
    private void setButtonLightEnabled(boolean enable){
        if(null == mLight){
            mLight = LocalServices.getService(LightsManager.class).getLight(LightsManager.LIGHT_ID_BUTTONS);
        }
        if(!isButtonLightInited){
            mButtonLightTimeout = /*mContext.getResources().getInteger(com.android.internal.R.integer.config_button_light_timeout_msec)*/4000;
            mButtonBrightness = /*mContext.getResources().getInteger(com.android.internal.R.integer.config_button_light_bright_level)*/2;
            isButtonLightInited = true;
        }
        
        if(mLightHandler.hasMessages(MSG_TURN_OFF_BUTTON_LIGHT)){
            mLightHandler.removeMessages(MSG_TURN_OFF_BUTTON_LIGHT);
        }
        if(enable){
            mLight.setBrightness(mButtonBrightness);
            mLightHandler.sendMessageDelayed(mLightHandler.obtainMessage(MSG_TURN_OFF_BUTTON_LIGHT),mButtonLightTimeout);
        }else{
            mLight.turnOff();
        }
    }
    ...
    private Handler mLightHandler = new Handler(){
        public void handleMessage(Message msg) {
            switch(msg.what){
                case MSG_TURN_OFF_BUTTON_LIGHT:
    ...
    /**
     * call this if double-tap home key.by zj.
     */
    private boolean isDoubleTap() {
       System.arraycopy(mHint, 1, mHint, 0, mHint.length - 1);
 	   mHint[mHint.length - 1] = SystemClock.uptimeMillis();
/* 	  Log.i("...test", "----- dispatchOpenCamera ----- isDoubleTap ----- 0 ---- mHint[1] = " + mHint[1]
 			  + " && mHint[0] = "+mHint[0] + " && SystemClock.uptimeMillis() = "+SystemClock.uptimeMillis());*/
 	   if (mHint[0] >= SystemClock.uptimeMillis() - ViewConfiguration.getDoubleTapTimeout()) {
 		   return true;
 	   }
 	   return false;
    }

    //### add this for jump to Camera2.
    private long[] mHint = new long[2];
    private final static String LAUNCHER_PACKAGE = "com.cooee.phenix";
    private final static String CAMERA_PACKAGE = "org.codeaurora.snapcam";
    private final static String CAMERA_CLASS = "com.android.camera.CameraActivity";
    /**
     * add this for double-tap home key to camera. by zj.
     */
    private void dispatchOpenCamera() {
       try {
            String packageName = ActivityManagerNative.getDefault().getTasks(1, 0).get(0).topActivity.getPackageName();
            if (LAUNCHER_PACKAGE.equals(packageName)) {
            	Intent intent = new Intent();
            	intent.setClassName(CAMERA_PACKAGE, CAMERA_CLASS);
            	intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            	mContext.startActivity(intent);
            }
        } catch (RemoteException e) {
