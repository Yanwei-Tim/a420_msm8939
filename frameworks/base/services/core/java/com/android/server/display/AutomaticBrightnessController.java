...
class AutomaticBrightnessController {
	...
	private final Spline mFolderclosedScreenAutoBrightnessSpline;
    private final boolean mIsDoubleScreen = SystemProperties.get("ro.config.doublescreen","0").equals("1");

	int[] FOLDER_CLOSED_LUX = {
		10,30,60,100,150,210,255
	};
	int[] FOLDER_CLOSED_BRIGHTNESS = {
    ...
    public AutomaticBrightnessController(Callbacks callbacks, Looper looper,
		...
		mFolderclosedScreenAutoBrightnessSpline = DisplayPowerController.createAutoBrightnessSpline(FOLDER_CLOSED_LUX,FOLDER_CLOSED_BRIGHTNESS);
    }
    ...
    private void updateAutoBrightness(boolean sendUpdate) {
        ...
        float value;
		if(mIsDoubleScreen && SystemProperties.get("persist.sys.folderopen","0").equals("0")){
			value = mFolderclosedScreenAutoBrightnessSpline.interpolate(mAmbientLux);
		}else{
			value = mScreenAutoBrightnessSpline.interpolate(mAmbientLux);
		}
