...
public class TaskPersister {
    ...
    /*
     * action:clear data fix bug of 14296
     * add by chr
     */
    private void cleardate()
   {
         int files_cont = 0;
         int image_cont = 0;
         File[] recent_Files = sTasksDir.listFiles(); 
         File[] recent_image = sImagesDir.listFiles();     
         if(recent_Files != null)
            files_cont =  recent_Files.length;

        if(recent_image != null)
        {
            image_cont = recent_image.length;
        }
        try{
            for(int i = 0; i < files_cont ; i ++)
            {
                recent_Files[i].delete();
            }   
            for(int i = 0; i < image_cont; i ++)
            {
                recent_image[i].delete();
            } 
        }catch(Exception e)
        {
