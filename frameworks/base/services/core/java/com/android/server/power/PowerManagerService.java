...
//added by wsl start
	      IntentFilter stkPulseFilter = new IntentFilter();
	      stkPulseFilter.addAction("com.sensortek.broadcast.pulse.enable");
	      stkPulseFilter.addAction("com.sensortek.broadcast.pulse.disable");
	      mContext.registerReceiver(new PulseReceiver(), stkPulseFilter, null, mHandler);
//added by wsl end	

           //add backreciever start by chr 
            filter = new IntentFilter();
            filter.addAction("audio_ftm_submic");
            mContext.registerReceiver(new Audiosubmic(), filter, null, mHandler);
            //end
...
//wsl add for a400 st3x1x heart rate start
    private final class PulseReceiver extends BroadcastReceiver {
      public void onReceive(Context context, Intent intent) {
         synchronized (mLock) {
                String value;
		boolean enableStatus = false;
		value = SystemProperties.get("persist.service.stk.pulse", "1");
		if(value.equals("1"))
		enableStatus = true;
		if(intent.getAction().equals("com.sensortek.broadcast.pulse.enable")) {
		Slog.e(TAG, "Pulse enable command received");
		if(enableStatus == false) {
		SystemProperties.set("persist.service.stk.pulse", "1");
		value = SystemProperties.get("persist.service.stk.pulse", "1");
		Slog.e(TAG, "After set, persist.service.stk.pulse=" + value);
		}
		else {
		Slog.e(TAG, "Pulse is already enabled");
		}
		}
		else if(intent.getAction().equals("com.sensortek.broadcast.pulse.disable")) {
		Slog.e(TAG, "disable command received");
		if(enableStatus == true) {
		SystemProperties.set("persist.service.stk.pulse", "0");
		value = SystemProperties.get("persist.service.stk.pulse", "0");
		Slog.e(TAG, "After set, persist.service.stk.pulse=" + value);
		}
                else {
                Slog.e(TAG, "Pulse is already disabled");
                 }
           }
         }
      }
}
//wsl add for a400 st3x1x heart rate end
    ...
    private boolean checkWorkSourceObjectId(int uid, WakeLock wl) {
     ...
     //start  add by chr merge from  8939_21214
      private final class Audiosubmic extends BroadcastReceiver {

    	@Override
    	public void onReceive(Context arg0, Intent intent) {
              if(intent.getAction().equals("audio_ftm_submic")){
            	    String swtch = intent.getStringExtra("switch");
                    SystemProperties.set("sys.ftmservice",swtch);
                 }
    	}
    }
    //end 
