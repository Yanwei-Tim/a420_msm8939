...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode,
    ...
    private final String ext_antaction = "com.lovdream.ext_antnofi"; 
	private final String updatenotifi = "com.lovdream.undatenotifi";
    
    private RemoteViews views;
    private Notification status = new Notification();
    private NotificationManager nm = null;
    private  final int EXT_ANT_TOUCHNO = 1;
    
    private   int type = 0;
    
    private Handler  mexthandler = new Handler(){
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        ...
        IntentFilter extantfilter = new IntentFilter();
        extantfilter.addAction(extouchaction);
        extantfilter.addAction(exrelaseaction);
        extantfilter.addAction(updatenotifi);
        context.registerReceiver(mExtAntReceiver, extantfilter);
    ...
    private BroadcastReceiver mDemoReceiver = new BroadcastReceiver() {
      ...
      private BroadcastReceiver mExtAntReceiver = new BroadcastReceiver(){

		@Override
		public void onReceive(Context context, Intent intent) {
			
			  String action = intent.getAction();
			  if(extouchaction.equals(action))
			  {
				  initNotification();
				  sendextNotification(); 
			  }else if(exrelaseaction.equals(action))
			  {
				  cancelextNotification();
			  }
			  
			  if(updatenotifi.equals(action))
			  {
				  
				 type = intent.getIntExtra("exttype", 0);
    ...
    {
    	String text = mContext.getString(R.string.insertant);
    	
    	if(type == 1)
    	{
    		text = mContext.getString(R.string.Access_gps);
    	}else if(type == 2)
    	{
    		text = mContext.getString(R.string.Access_wifi);
    	}else if(type == 3)
    	{
    		text = mContext.getString(R.string.Access_gpswifi);
    	}
    	...
    	if(nm == null)
    	{
    	 ...
    	 views = new RemoteViews(mContext.getPackageName(), R.layout.extanttouch);
    	 
    	/* Intent toggleIntent =  Intent.makeRestartActivityTask(new ComponentName("com.android.systemui",
                 "com.android.systemui.settings.EXTantActivity")); */
    	 
    	 Intent toggleIntent = new Intent();
    	 toggleIntent.setClassName("com.android.settings",
                 "com.android.settings.deviceinfo.EXTantActivity");
    	 Settings.System.putInt(mContext.getContentResolver(), "extanttype", type);
    	// toggleIntent.putExtra("extanttype", type);
    	 toggleIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
    	 
         PendingIntent togglePendingIntent = PendingIntent.getActivity(mContext, 0, toggleIntent, 0);
         
//         Intent exitIntent = new Intent("com.lovdream.exitnofi");
//         PendingIntent exitPendingIntent = PendingIntent.getBroadcast(mContext,
//                 0 /* no requestCode */, exitIntent, 0 /* no flags */);
//        		
    	// views.setOnClickPendingIntent(R.id.ext_ant, togglePendingIntent);
    	
         NotificationCompat.Builder status1 = new NotificationCompat.Builder(
        		 mContext);
     //    status1.setContent(views)
         status1.setContentIntent(PendingIntent.getActivity(mContext, 0, toggleIntent, 0));
//         status1.setDeleteIntent(exitPendingIntent);
         status1.setSmallIcon(R.drawable.extant)
         .setTicker(mContext.getString(R.string.insertant) )
         .setContentTitle(mContext.getString(R.string.insertant) )
         .setContentText(text);
    ...
    private void sendextNotification()
    {
    	Log.i(TAG, "=============sendextNotification=================");
    	if(nm == null)
    	{
    	  nm = (NotificationManager)mContext.getSystemService(Context.NOTIFICATION_SERVICE);
    	}
     
        nm.notify(EXT_ANT_TOUCHNO, status);
    }
    
    private void cancelextNotification()
    {
    	Log.i(TAG, "=============cancelextNotification=================");
    	if(nm == null)
    	{
    	  nm = (NotificationManager)mContext. getSystemService(Context.NOTIFICATION_SERVICE);
    	}
    	 nm.cancel(EXT_ANT_TOUCHNO);
