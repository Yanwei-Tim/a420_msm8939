...
public class CellularTile extends QSTile<QSTile.SignalState> {
    ...
    protected void handleClick() {
     /*   MetricsLogger.action(mContext, getMetricsCategory());
    	...
    	boolean	state = mDataController.isMobileDataSupported()? mDataController.isMobileDataEnabled() : false;
    ...
    protected void handleUpdateState(SignalState state, Object arg) {
        ...
        boolean  state_date = mDataController.isMobileDataSupported()? mDataController.isMobileDataEnabled() : false;

        int iconId = state_date? R.drawable.ic_qs_data_on: R.drawable.ic_qs_data_off;
        //end 
        
       //start fix bug of 12259  add by chr
         if(cb.airplaneModeEnabled == true)
            {
            	airplaneMode = true;
            	iconId = R.drawable.ic_qs_data_off;
            }
            else 
            {
            	airplaneMode = false;
        ...
        state.overlayIconId = 0; //cb.enabled && (cb.dataTypeIconId > 0) ? cb.dataTypeIconId : 0;
        state.filter =  true;// iconId != R.drawable.ic_qs_no_sim;
        state.activityIn  = false; //cb.enabled && cb.activityIn;
        state.activityOut = false; //cb.enabled && cb.activityOut;

        state.label =  r.getString(R.string.quick_settings_mobile_data_label); 
