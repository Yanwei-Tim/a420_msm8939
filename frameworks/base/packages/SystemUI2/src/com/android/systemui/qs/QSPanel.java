...
public class QSPanel extends ViewGroup {
    private static final float TILE_ASPECT = 0.74f;

    private final Context mContext;
    protected final ArrayList<TileRecord> mRecords = new ArrayList<TileRecord>();
    protected final ArrayList<TileRecord> mRecords_disply = new ArrayList<TileRecord>();
    ...
    private ViewGroup lgRelateView;
    private LinearLayout  lgLayout;
    private HorizontalScrollView ScrollView;
    
    private LinearLayout search_connetLayout ;
    private View  searchGroup ;

    private View lgqsGroup;
    private ViewGroup  frame_view;
    
    private int hidescrollview = 0;
  
    private Collection<QSTile<?>> mCollection;
    
    private Intent BLUETOOTH_SETTINGS = new Intent(Settings.ACTION_BLUETOOTH_SETTINGS);
    private List<Map<String, Object>> data_list = new ArrayList<Map<String,Object>>();
    private SimpleAdapter mAdapter;

    private int row_num = 5;
    private int record_extra = 0;
    private int mQsGroupheight;
    ...
    public QSPanel(Context context, AttributeSet attrs) {
        ...
        ScrollView =  (HorizontalScrollView)  LayoutInflater.from(context).inflate(R.layout.extend_listviewlg, this,false);
        addView(ScrollView);
        
        searchGroup =  LayoutInflater.from(context).inflate(R.layout.search_qspanel, this, false);
        addView(searchGroup);
        search_connetLayout = (LinearLayout)searchGroup.findViewById(R.id.search_connect);      
        lgLayout = (LinearLayout) findViewById(R.id.lgliner);
        
        lgqsGroup = LayoutInflater.from(context).inflate(R.layout.lgqsgroup, this, false);
        addView(lgqsGroup);
        frame_view = (ViewGroup)lgqsGroup.findViewById(R.id.frame_view);
        ...
        mBrightnessController = new BrightnessController(getContext(),
                ...
                (ToggleSlider) findViewById(R.id.brightness_slider),(CheckBox) findViewById(R.id.auto_birght),(TextView) findViewById(R.id.auto_text));
        ...
        search_connetLayout.setOnClickListener(new OnClickListener(){

			@Override
			public void onClick(View v) {
			
				Scrrenshot(KeyEvent.KEYCODE_BACK);
				BLUETOOTH_SETTINGS.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				mContext.startActivity(BLUETOOTH_SETTINGS);
			}
    ...
    public void setScrollView(int flag)
    {
    	hidescrollview = flag;
    	if(flag == 1)
    	{
	    	...
	    	if(ScrollView != null)
	    	{
	    		...
	    		lgqsGroup.setVisibility(View.VISIBLE);
	    		refreshAllTiles_dis();
	    	//	updateResources_dispaly();
    		...
    		if(ScrollView != null)
	    	{
	    		...
	    		lgqsGroup.setVisibility(View.GONE);
	    		refreshAllTiles();
	    		updateResources();
	    	}
		}
    ...
    private void Scrrenshot(final int keyCode) {
    ...
    public void updateResources() {
        ...
        mQsGroupheight  = res.getDimensionPixelSize(R.dimen.qs_groupheight);
        record_extra    = res.getDimensionPixelSize(R.dimen.qs_record_extra);
        if (mColumns != columns) {
            mColumns = columns;
            postInvalidate();
        }
    ...
    public void updateResources_dispaly() {
        final Resources res = mContext.getResources();
        final int columns = Math.max(1, res.getInteger(R.integer.quick_settings_num_columns));
        mCellHeight = res.getDimensionPixelSize(R.dimen.qs_tile_height);
        mCellWidth = (int)(mCellHeight * TILE_ASPECT);
        ...
        for (TileRecord r : mRecords_disply) {
            r.tile.clearState();
        }
        if (mListening) {
            refreshAllTiles_dis();
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
        ...
        int count2 = mRecords_disply.size();
        for (int i = 0; i < count2; i++) {
            View detailView = mRecords_disply.get(i).detailView;
            if (detailView != null) {
                detailView.dispatchConfigurationChanged(newConfig);
            }
        }
    ...
    public void refreshAllTiles_dis() {
    ...
    public void setTiles_display(Collection<QSTile<?>> tiles) {
        for (TileRecord record : mRecords_disply) {
            removeView(record.tileView);
        }
        mRecords_disply.clear();
        for (QSTile<?> tile : tiles) {
            addTile(tile, 0);
        }
        if (isShowingDetail()) {
    ...
    private void addTile(final QSTile<?> tile, int type) {
        ...
        if(type == 1)
        {
	        ...
	        if(lgLayout != null)
	        {
	        	lgLayout.addView(r.tileView, r.tileView.getWidth(), r.tileView.getHeight());
	        }
        }else {
        	mRecords_disply.add(r);
			if(frame_view != null)
			{
				frame_view.addView(r.tileView, r.tileView.getWidth(), r.tileView.getHeight());
			}
		}
    ...
    private void setGridContentVisibility(boolean visible) {
        ...
        for (int i = 0; i < mRecords_disply.size(); i++) {
            TileRecord tileRecord = mRecords_disply.get(i);
            if (tileRecord.tileView.getVisibility() != GONE) {
                tileRecord.tileView.setVisibility(newVis);
            }
        }
    ...
    public boolean isdisland()
    {
    	if(row_num >5 && lgqsGroup.isShown())
    	{
    		return true;
    	}else {
			return false;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        row_num = width / (mCellWidth +record_extra);
	    ...
	    for (TileRecord record : mRecords) {
	         ...
	         //   if(++i > 10) continue; 
	            if (record.tileView.getVisibility() == GONE) continue;
	            final int cw = mCellWidth; // record.row == 0 ? mLargeCellWidth : mCellWidth;
	            final int ch = mCellHeight; // record.row == 0 ? mLargeCellHeight : mCellHeight;
        ...
        if(search_connetLayout != null)
        {
        	search_connetLayout.measure(exactly(width), 84);
        	h += 55;
        }
        
        if(hidescrollview == 1)
        {
        	h += mQsGroupheight - 196;
        }
        mGridHeight = h;
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        int lgqsGroupheight = 0;
        int offset = 125; 
        int lgLayoutwidth = 1470;
        
        if(hidescrollview != 1)
        {
        	scrollviewheight = 196;
        }else{
        	lgqsGroupheight = mQsGroupheight;
        }
        
        if(ScrollView != null)
        {
        	ScrollView.layout(0, 0, w, scrollviewheight);
        	lgLayout.layout(0, 0, lgLayoutwidth, scrollviewheight);
        }
        
        if(lgqsGroup != null)
        {
        	lgqsGroup.layout(0, 0, w, lgqsGroupheight);
        	
        }
        if(frame_view != null)
        {
        	frame_view.layout(0, 0, w, lgqsGroupheight);
        }
        int padding = mBrightnessPaddingTop + scrollviewheight + lgqsGroupheight;
        if (isVoicePrefEnabled()) {
            if (mSimSwitcherView.getVisibility() != View.GONE) {
                padding = mBrightnessPaddingTop*2 + mSimSwitcherView.getMeasuredHeight() + scrollviewheight;
            }
        ...
        boolean isRtl = false;// getLayoutDirection() == LAYOUT_DIRECTION_RTL;
        ...
        for (TileRecord record : mRecords) {
            ...
            final int cw = mCellWidth;// record.row == 0 ? mLargeCellWidth : mCellWidth;
            final int extra = record_extra;// (w - cw * cols) / (cols + 1);
            int left = record.col * cw + (record.col + 1) * extra;
            final int top = 5;// getRowTop(record.row);
            int right;
        ...
        if(search_connetLayout != null)
        {
        	...
        	int	height = padding + mBrightnessView.getMeasuredHeight() + 10;		
        	int	botom = height + 84;
//        	if(hidescrollview == 1)
//        	{
        		height = padding + mBrightnessView.getMeasuredHeight() ;	
//        	}
        	search_connetLayout.layout(0,height , getWidth(), 
        			botom );
        }
    ...
    private int getRowTop(int row) {
        if (!isVoicePrefEnabled() || mSimSwitcherView.getVisibility() == View.GONE) {
            ...
            return mBrightnessView.getMeasuredHeight() + mBrightnessPaddingTop
                    + 186; //mLargeCellHeight - mDualTileUnderlap + (row - 1) * mCellHeight;
        } else {
