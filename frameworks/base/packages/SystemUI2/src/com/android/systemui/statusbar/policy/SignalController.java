...
public abstract class SignalController<T extends SignalController.State,
    ...
    public int getQsCurrentIconId() {
        if (mCurrentState.connected) {
        	...
        	if(level > 4)
        		level = 4;
        	Log.i(TAG, "getQsCurrentIconId level = " + level);
        	//end
            return getIcons().mQsIcons[mCurrentState.inetCondition][level/*mCurrentState.level */];
    ...
    public int getCurrentIconId() {
        if (mCurrentState.connected) {
        	...
        	Log.i(TAG, "getCurrentIconId level = " + level);
        	//end
            return getIcons().mSbIcons[mCurrentState.inetCondition][level /*mCurrentState.level */];
        } else if (mCurrentState.enabled) {
